...
func TestUser_GetByEmail(t *testing.T) {
	...
	if u.ID == 0 {
		t.Error("user returned id of 0: ", err)
	}
}
...
func TestToken_GetUserFromToken(t *testing.T) {
	token := "abc"

	_, err := models.Tokens.GetUserFromToken(token)
	if err == nil {
	...
	_, err = models.Tokens.GetUserFromToken(u.Token.PlainText)
	if err != nil {
...
func TestToken_GetByToken(t *testing.T) {
	...
	if err == nil {
		t.Error("error getting non existent token by token")
	}
}
...
var authData = []struct {
	...
	{"invalid", "some-invalid-token", "a@here.com", true, "invalid token accepted as valid"},
	{"invalid_length", "some-invalid-token", "a@here.com", true, "token of wrong length accepted valid"},
	{"no_user", "some-invalid-token", "a@here.com", true, "no user, but token accepted as valid"},
...
func TestToken_BadHeader(t *testing.T) {
	...
	_, err = models.Tokens.AuthenticateToken(req)
	if err == nil {
		t.Error("failed to catch token for deleted token")
	}
}
...
func TestToken_GivenTokenValid(t *testing.T) {
	u, err := models.Users.GetByEmail(dummyUser.Email)
	if err != nil {
		t.Error(err)
	}
	...
	ok, _ = models.Tokens.ValidToken("invalid-token")
	...
	if err == nil {
		t.Error("no error when expecting an error")
	}
	if ok {
