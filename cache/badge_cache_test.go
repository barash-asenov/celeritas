...
func TestBadgerCache_Forget(t *testing.T) {
	...
	inCache, err := testBadgerCache.Has("foo")
	if inCache {
		t.Error("foo found in cache, but should have been forgotten")
	}
}
...
func TestBadgerCache_Empty(t *testing.T) {
	...
	if inCache {
		t.Error("alpha found in cache, but should have been forgotten")
	}
}
...
func TestBadgerCache_EmptyByMatch(t *testing.T) {
	...
	if inCache {
		t.Error("alpha2 found in cache, but should have been forgotten")
	}

	inCache, err = testBadgerCache.Has("beta")
	if !inCache {
		t.Error("beta not found in cache, but should be there")
	}
}