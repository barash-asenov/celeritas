...
func TestRedisCache_Has(t *testing.T) {
	...
	if inCache {
		t.Error("foo found in cache, and it is actually not there")
	}
	...
	if !inCache {
		t.Error("foo not found in cache, it should be in there")
	}
}
...
func TestRedisCache_Forget(t *testing.T) {
	...
	if inCache {
		t.Error("alpha found in cache, it should not be there")
	}
}
...
func TestRedisCache_EmptyByMatch(t *testing.T) {
	...
	if inCache {
		t.Error("alpha2 found in cache, it should not be there")
	}
	...
	if !inCache {
		t.Error("beta should still be in there, but its not")
	}
}
